...
Public Function RunFile(argFileName As String)
    
' Run .VBS file from access e.g. RunVBS "\\vla31\frontends$", "Install TSE Archive_new.vbs"
' also runs other files, like .PDF
    ...
    p = Len(argFileName)
    Do While Mid$(argFileName, p, 1) <> "\" And p > 0: p = p - 1: Loop
    If p = 0 Then
        MsgBox "No path name specified"
        Exit Function
    End If
    
    strPath = Left$(argFileName, p)
    strFN = Mid$(argFileName, p + 1)
    ...
    lReturn = ShellExecute(hWnd, lpOperation, strFN, lpParameters, strPath, vbNormalFocus)    'vbMaximizedFocus)
End Function
