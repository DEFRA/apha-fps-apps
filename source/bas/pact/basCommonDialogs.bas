...
Option Explicit
    ...
    Private Const BIF_UAHINT = &H100                'Add a UA hint to the dialog, in place of the edit box.
    ...
    Private Const BIFDONTGOBELOWDOMAIN = &H2
    Private Const BIF_USENEWUI = &H50               ' = BIF_NEWDIALOGSTYLE +BIF_EDITBOX
    Private Const BIF_BROWSEINCLUDEURLS = &H80
    Private Const BIF_NONEWFOLDERBUTTON = &H200     'Do not add the "New Folder"
...
Function FindFile(strTitle, strSearchPath, FileSpecs As String, Optional strInitialFile = "") As String
...
' e.g. strFileName = findfile("Open File", "c:\", "Program Files|*.exe;*.com|All Files|*.*")
    ...
    If Not IsNull(strInitialFile) Then msaof.strInitialFile = strInitialFile
...
'            .ulFlags = BIFRETURNONLYFSDIRS + BIFDONTGOBELOWDOMAIN + BIF_USENEWUI
'        End With
