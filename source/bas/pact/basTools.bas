...
Dim gstrSQL As String, gstrSelectDescription As String
...
Function DoReport(DOCNAME, SelectionCriteria, mode) As Integer

' Mode should = A_NORMAL (0) or A_PREVIEW (2)
...
DoCmd.OpenReport DOCNAME, mode, , SelectionCriteria
...
Function GoFirst(Optional fn As Variant = "")
On Error Resume Next
Err = 0
If fn & "" = "" Then
...
Function GoLast(Optional fn As Variant = "")
On Error Resume Next
Err = 0
If fn & "" = "" Then
...
Function GoNext(Optional fn As Variant = "")
On Error Resume Next
Err = 0
If fn & "" = "" Then
...
Function GoPrevious(Optional fn As Variant = "")
On Error Resume Next
Err = 0
If fn & "" = "" Then
...
Function loadForm(DOCNAME, Optional linkcriteria, Optional WindowMode, Optional CommandString As Variant) As Integer
' WindowMode = a_normal (0), a_hidden (1), a_icon (2), a_dialog (3)

Dim activeFM As String

On Error Resume Next

On Error GoTo Err_LoadForm
If IsMissing(linkcriteria) Then linkcriteria = ""
If IsMissing(WindowMode) Then WindowMode = 0
If IsMissing(CommandString) Then CommandString = ""
...
Select Case Err
    ...
    Case 2102
        MsgBox "Sorry, function not yet available", 48, "PACS"
        loadForm = False
...
Function validate(ctrl As Control, validation As String, ctrlName, Optional depCtrl As Control, Optional depCtrlName) As Boolean
...
Dim eMsg As String, lf As String
...
If eMsg <> "" Then
    ...
    If ctrl.Enabled And ctrl.Visible Then
        ...
        ctrl.SetFocus
    End If
    msgDataError eMsg & vbNewLine & vbNewLine & "Note: Changes can be undone by pressing the Esc key"
End If
...
Static Function SettingsGet(itemName As String, Optional Default As Variant, Optional CheckRecursive = True) As Variant
...
Select Case DataErr
    Case 3162: msgFormError "Blanks not allowed": Response = acDataErrContinue
    ...
    Case 7786: msgFormError "This data item cannot be deleted as it is referred to by other data": Response = acDataErrContinue
    Case Else
End Select
...
Public Function RequeryAllLists(Optional varFormName As Variant, Optional varFormObject As Variant)
' Finds all combos and lists in the specified form
' and requeries them.
' Also, if the form contains subforms, it does the same for them
' Either formname or form object can be passed argument

Dim frm As Form, ctrl As Variant

Const ProcID = "RequeryAllLists"
On Error GoTo ErrorHandler


If IsMissing(varFormName) Then
    Set frm = varFormObject
Else
...
' call this function from an OnKeyPress event to drop a combo on first keypress
...
Public Function MultiSelectGet(varList As Variant, lngListCount As Long, lstMulti As ListBox, Optional varDelimiter As Variant = "", Optional varColumnNo As Variant = 0) As Boolean
...
Public Function addToPickList(strNewData As String, strTableName As String, strDataField As String) As Integer
'e.g. Response = addToPickList(NewData, "tlkpProjects", "ProjectCode")

On Error GoTo Err_proc

Dim lngLookupID As Long
Dim rs As Recordset

addToPickList = acDataErrDisplay
If MsgBox("'" & strNewData & "' is not in the pick list" & vbNewLine & "Do you want to add it?" & vbNewLine & "(please be sure it's not already in the list already with different spelling, spacing or punctuation)", vbYesNo, "Not in pick list!") = vbYes Then
...
Public Function getChildData(strChildTable As String, strDataFields As String, strJoinData As String) As Variant
...
Public Function ClearControl(ctrl As Control)

If TypeOf ctrl Is ListBox Then
    If ctrl.MultiSelect <> 0 Then
        ClearMultiSelect ctrl
        Exit Function
    End If
ElseIf TypeOf ctrl Is CheckBox Then
    If ctrl.TripleState Then
        ctrl = Null
    Else
        ctrl = False
    End If
...
' Used when checking that a record with the same key value exists in the dataset
    ...
    If Not (.EOF And .BOF) Then
        ...
        Else
            If blnCheckSoundsLike Then
                ...
                .MoveFirst
CheckSoundsLike:
                .FindNext "soundex([" & DescriptionFieldName & "]) = """ & Soundex(DescriptionFieldValue) & """"
                If Not .NoMatch Then
                    'If CStr(.Bookmark) = CStr(frm.Bookmark) Then GoTo CheckSoundsLike
...
Public Function LookupIDExists(Optional blnCheckSoundsLike As Boolean = True) As Boolean
' Used when checking that a record with the same ID exists in the dataset
Dim rs As Recordset, blnResults As Boolean, frm As Form
Dim DescriptionFieldName, DescriptionFieldValue
...
' The supplied defualt will be 'evaluated',
...
Static Function BESettingsGet(itemName As String, Optional Default As Variant) As Variant
' If RunMode = "Test" in tblSettings, this function
' will return the values from the TestSetting column if present (or the Setting column if TestSetting is null),
...
If Not blnInitialised Then
    blnInitialised = True
    Set rs = CurrentDb.OpenRecordset("tblSettings", dbOpenDynaset)
End If
'rs.Requery
rs.FindFirst "Id = ""RunMode"""
If rs.NoMatch Then
...
Do While InStr(Setting, "[") > 0
    ...
    Do While pe <= Len(Setting)
        ...
        End Select
        If bracketCount = 0 Then
            subSetting = BESettingsGet(Mid$(Setting, ps + 1, pe - (ps + 1))) & ""
            Setting = Left$(Setting, ps - 1) & subSetting & Mid$(Setting, (pe + 1))
            Exit Do
        End If
        pe = pe + 1
...
Function BESettingsSet(itemName As String, itemValue As Variant)
Dim iv As Variant
...
If DCount("*", "tblSettings", "id = " & SQLstring(itemName)) = 0 Then
    runActionQuery "INSERT INTO [tblSettings] ( id ) values (" & SQLstring(itemName) & ");", "SettingsSet"
End If
If runActionQuery("UPDATE [tblSettings] SET setting = " & iv & " where Id = " & SQLstring(itemName) & ";", "SettingsSet") Then
End If
