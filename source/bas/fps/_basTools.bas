...
'Declare Function apiGetWindowRect Lib "user32" Alias "GetWindowRect" (ByVal hwnd As Long, lpRect As WindowRectType) As Long
...
End Function
Sub ArraySort(anyArray() As Variant, AorD As String)
...
Public Function Split(varInput As Variant, varDelimiters As Variant, ParamArray itemArray()) As Boolean
' Splits varInput into items separated by any of the characters in varDelimiters
' and returns the trimmed items in the itemArray() paramarray

Dim p As Integer, i As Integer, item As String, intItemCount As Integer

item = ""
i = LBound(itemArray())
intItemCount = UBound(itemArray())

For p = 1 To Len(varInput & "")

    If i > intItemCount Then Exit Function
    
    ' check for a delimiter
    
    If InStr(varDelimiters, Mid$(varInput, p, 1)) = 0 Then
        item = item & Mid$(varInput, p, 1)
    Else
...
If item <> "" And i <= intItemCount Then
    itemArray(i) = Trim(item)
    i = i + 1
End If

Split = (i = intItemCount + 1)
...
Function emptyTable(tableName, Optional varCriteria = Null) As Integer


emptyTable = runActionQueryCmd("delete * from [" & tableName & "]" & IIf(IsNull(varCriteria), "", " where " & varCriteria) & ";", "emptyTable " & tableName)
...
c = 0
ReDim FNames(1 To 1) As Variant
...
Function getFileList(Filespec As String, FNames() As Variant, FNameList As String) As Integer
...
Function loadForm(docname, Optional linkcriteria = "", Optional Normal0Icon2Dialog3 = 0, Optional CommandString As Variant = "", Optional FormStyle_0Normal_3DS = acNormal) As Integer
...
End Select
    ...
    If linkcriteria = "NewRecord" Then
        DoCmd.OpenForm docname, FormStyle_0Normal_3DS, , , acFormAdd, Normal0Icon2Dialog3, CommandString & ""
    Else
        If IsNull(FormMode) Then
            DoCmd.OpenForm docname, FormStyle_0Normal_3DS, , linkcriteria, , Normal0Icon2Dialog3, CommandString & ""
        Else
            DoCmd.OpenForm docname, FormStyle_0Normal_3DS, , linkcriteria, FormMode, Normal0Icon2Dialog3, CommandString & ""
        End If
    End If
    loadForm = True
...
Sub msgInfo(msg, Optional varTitle = Null)

    MsgBox msg, 64, Nz(varTitle, "Information")
...
' Copies the value of the current field into all null fields in subsequent records
...
With RS
    Do While Not .EOF
        .MoveNext
        If Not .EOF Then
            If IsNull(RS(ctrl.ControlSource)) Then
                .Edit
                    RS(ctrl.ControlSource) = ctrl
                .Update
            Else
                Exit Do
            End If
...
'ws.BeginTrans
db.Execute SQLst, dbSeeChanges + dbFailOnError
...
runActionQuery_err:
    ...
    msgSystemError sysErr, sysErrMsg & "SQL failed..." & vbNewLine & vbNewLine & SQLst, IIf(ID = "", "runActionQuery", ID)
    'ws.Rollback
    runActionQuery = False
...
End Function
Function SQLnumber(xx, Optional blnAllowNull = True) As String
...
VersionNo = DMax("Version", "tblVersions", "")
End Function
...
For p = 2 To Len(s1)
    ...
    If c > 0 Then
        ...
        If x <> " " And x <> lastX Then
            c1 = c1 & x
            lastX = x
        End If
        If Len(c1) = SOUNDEX_LENGTH Then p = Len(s1) + 1
    End If
Next

Soundex = Left$(c1 & String(SOUNDEX_LENGTH, "0"), SOUNDEX_LENGTH)
...
Public Function GetNumberPrefix(varText) As Variant
' Extracts a number from the start of a string and returns it

If IsNull(varText) Then
    GetNumberPrefix = Null
    Exit Function
End If
If IsNumeric(Left$(varText, 1)) Then
    GetNumberPrefix = Val(replaceChars(varText, " ", "_"))
Else
    GetNumberPrefix = Null
End If
End Function
...
Public Function GetAlphaSuffix(varText) As Variant
' Extracts characters after the numeric prefix
Dim s As Integer, varPrefix As Variant
If IsNull(varText) Then
    GetAlphaSuffix = Null
    Exit Function
End If

varPrefix = Nz(GetNumberPrefix(varText), "")
GetAlphaSuffix = Mid$(varText, Len(varPrefix) + 1)
...
Public Function TableDesign(Optional varTableName = Null)
Dim strTableName As String

On Error GoTo ErrorHandler

If IsNull(Screen.ActiveControl) Then Exit Function

If Nz(varTableName, "") = "" Then
    strTableName = Screen.ActiveControl
Else
    strTableName = varTableName
End If

DoCmd.SetWarnings False
If isTable(strTableName) Then
    DoCmd.OpenTable strTableName, acViewDesign
ElseIf isQuery(strTableName) Then
    DoCmd.OpenQuery strTableName, acViewDesign
End If
DoCmd.SetWarnings True
Exit Function
...
Else
    MsgBox "Err " & CStr(Err) & ": " & Error$
End If
End Function

Function isTable(strTableName) As Integer
Dim db As Database, ctr As TableDef
On Error GoTo fini

isTable = False
Set db = CurrentDb()
Set ctr = db.TableDefs(strTableName)
isTable = True

fini:
End Function

Public Function TableView(Optional varTableName = Null)
Dim strTableName As String
...
ElseIf isQuery(strTableName) Then
    DoCmd.OpenQuery strTableName, acViewNormal
End If
...
Dim DescriptionFieldName, DescriptionFieldValue, DescriptionFieldOldValue
...
DescriptionFieldOldValue = Screen.ActiveControl.OldValue
    ...
    If Not (.EOF And .BOF) Then
        ...
        Else
            If blnCheckSoundsLike Then
                ' check with soundex"
                .FindFirst "soundex([" & DescriptionFieldName & "]) = """ & Soundex(DescriptionFieldValue) & """" & IIf(DescriptionFieldOldValue & "" = "", "", " and [" & DescriptionFieldName & "] <> " & SQLstring(DescriptionFieldOldValue))
                If Not .NoMatch Then
                    If MsgBox("An item similar to this already exists (" & .Fields(DescriptionFieldName) & ")" & vbNewLine & _
                              "If you want to continue, click OK, otherwise click Cancel then press Escape.", vbInformation + vbOKCancel, "Possible Duplicate Entry") = vbCancel Then
                              blnResults = True
                    End If
                End If
            End If
...
With RS
    ...
    lngDS2 = Nz(.Fields(strSeqColumn), .Fields(strIDcolumn))
    .Edit
...
Public Function CreateIndex(tableName, FieldNameList)
' Creates an index on a table.
' Used to add an index to a SQLServer view

' CREATE UNIQUE INDEX vAnimal_AllDetails_PK ON vAnimal_AllDetails(AnimalID)

On Error GoTo ErrorHandler

DoCmd.RunSQL "CREATE UNIQUE INDEX " & tableName & "_PK on " & tableName & "(" & FieldNameList & ")"
...
Public Function getChildJoin(strMasterTable As String, strJoinFields As String) As String
' Return SQL to allow selection criterion to be linked to master table, e.g.
' strMasterTable = "tblNeuroExam"
' strJoinFields = "AnimalID,ObservationDate"
' returns " and [AnimalID] = tblNeuroExam.[AnimalID] and [ObservationData] = tblNeuroExam.[ObservationDate]"

Dim arJF() As Variant, i As Integer, n As Integer, strSQL As String
i = breakApart(strJoinFields, ",", arJF)

strSQL = ""
For n = 1 To i
    strSQL = strSQL & " and [" & arJF(n) & "] = [" & strMasterTable & "].[" & arJF(n) & "]"
Next
getChildJoin = strSQL
...
Public Function GlobalColumnEdit(varTableName, varIDfieldname, varOldValue, varNewValue) As Boolean

' UPDATE &tbl SET &id = &newval where &id = &oldval
Dim strSQL As String
Dim varOldVal, varNewVal

On Error GoTo ErrorHandler

If TypeName(varOldValue) = "string" Or TypeName(varNewValue) = "string" Then
    varOldVal = SQLstring(varOldValue)
    varNewVal = SQLstring(varNewValue)
Else
    varOldVal = CStr(varOldValue)
    varNewVal = CStr(varNewValue)
End If

strSQL = "UPDATE [" & varTableName & "] SET [" & varIDfieldname & "] = " & varNewVal & " WHERE [" & varIDfieldname & "] = " & varOldVal & ";"
Debug.Print strSQL

GlobalColumnEdit = runActionQuery(strSQL, "GlobalColumnEdit")

Exit Function

ErrorHandler:

    msgSystemError Err, Error$ & vbNewLine & vbNewLine & strSQL, "GlobalColumnEdit"
    GlobalColumnEdit = False
...
Public Function PrintWithDialog()
On Error Resume Next
DoCmd.RunCommand acCmdPrint
...
Function WebDisplay(strURL, Optional blnSeparateWindow = True) As Boolean
Dim msgTxt
Dim errorItem As Error
...
msgTxt = "Err: " & Err & " " & Error$ & vbNewLine & strURL

If Err = -2146697210 Then
    MsgBox "Invalid URL (" & strURL & ")"
    Exit Function
End If
...
For Each errorItem In DBEngine.Errors
     With errorItem
         msgTxt = msgTxt & "Err: " & .Number & " " & .Description & " (" & .Source & ") & vbNewLine"
     End With
Next errorItem

MsgBox msgTxt, vbCritical, "Error"

End Function

Sub testit()

'WebDisplay "http://VLA49/histopathologysystem/ViewSamples.aspx?PGNumber=" & "PG0001/03"
'WebDisplay "http://google.co.uk" ' works OK!
WebDisplay "http://multimap.com/map/browse.cgi?lat=50.8344&lon=-0.2767&scale=100000"
End Sub
