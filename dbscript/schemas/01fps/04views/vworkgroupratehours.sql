...
CREATE OR REPLACE VIEW fps.vworkgroupratehours AS
 SELECT DISTINCT timecostcalcs_1.workgroup,
   ...
   FROM (((fps.timecostcalcs
     JOIN fps.timecodevalid ON (((timecostcalcs.workgroup)::text = (timecodevalid.workgroup)::text)))
     JOIN fps.timecostcalcs timecostcalcs_1 ON (((timecodevalid.parentproject)::text = (timecostcalcs_1.project)::text)))
