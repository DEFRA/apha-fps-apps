...
CREATE OR REPLACE VIEW fps.vpostmort1 AS
 SELECT tlkptestcapability.planportfolio,
   ...
   FROM (((fps.vtbltestrequ
     ...
     LEFT JOIN fps.qrytestspccostplan_xtab ON (((testorproduct.itemcode)::text = (qrytestspccostplan_xtab.testcode)::text)))
  WHERE (monthlyoutput.month <= ( SELECT max(tblperiod.endperiod) AS endperiod
           FROM fps.tblperiod
          WHERE (tblperiod.finalsummariesrun = '-1'::integer)))
  GROUP BY tlkptestcapability.planportfolio, monthlyoutput.testcode, testorproduct.shortdescription, qrytestspccostplan_xtab.labt, qrytestspccostplan_xtab.vetr, monthlyoutput.workgroup
