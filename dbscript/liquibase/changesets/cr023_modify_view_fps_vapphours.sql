...
--changeset repo-admin:CR023 labels:ddl context:all 

CREATE OR REPLACE VIEW fps.vapphours
 AS
 SELECT tblwgemployee.workgroupgrade,
  ...
  WHERE tlkpproject.program::text <> 'ZT_Prog'::text AND lower(tlkpproject.projectstatus::text) = 'approved'::text AND tlkpprogram.sector_name::text = 'Charge'::text
