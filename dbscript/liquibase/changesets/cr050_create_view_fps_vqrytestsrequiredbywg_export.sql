...
CREATE OR REPLACE VIEW fps.vqrytestsrequiredbywg_export AS
WITH

-- Pre-calculate rolling 12-month window bounds (replaces DECLARE + SELECT TOP 1)
latest_release AS (
    SELECT
        year,
        latestmonthreleased,
...
-- mo.YearMonth computed column not yet in PostgreSQL; inlined as year*100+month
last_year_tests AS (
    SELECT
...
-- Replaces correlated DLookUp subquery; rn=1 picks first alphabetical workgroup per test
...
-- qryTestsRequiredThisYear: early filter on norequired <> 0 reduces downstream rows
tests_required AS (
    SELECT
        t.testcode,
...
-- qryTestsRequiredByWG_RCCost: RC cost price per TestCode expanded to WorkGroup
rc_cost AS (
    SELECT
        rc.testcode,
        wg.workgroup,
...
GROUP BY wg.profitcentre, wg.workgroup, t.testcode, t.itemdescription ,t.projectedtotal, t.unitprice
ORDER BY wg.profitcentre, t.testcode;

--ROLLBACK
--DROP VIEW IF EXISTS fps.vqrytestsrequiredbywg_export;
