...
namespace Apha.Costbook.DataAccess.Logging
{
    /// <summary>
    ...
    /// Keeps all file/DB I/O off the query and request hot paths.
    /// </summary>
    public sealed class PerformanceLogWriter : BackgroundService
    {
        ...
        private async Task FlushAsync(List<PerformanceLog> batch, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var context = scope.ServiceProvider.GetRequiredService<CostbookDbContext>();

                context.PerformanceLogs.AddRange(batch);
                await context.SaveChangesAsync(cancellationToken);

                // Detach so entities are not tracked beyond this write.
            ...
            {
                // Never let diagnostics logging crash the writer loop.
