/*
 * TRANSFORMENGINE MIGRATION — MaintenanceSettingsServiceTests.cs
 * Pattern  : stack-upgrade/msaccess-frm-to-dotnet10-mvc-e2e  Phase 13 — Unit Tests - Backend + Frontend xUnit Coverage
 * Migrated : 2026-06-23
 *
 * CHANGED:
 *   - New xUnit test class for Apha.Costbook.Application.Services.MaintenanceSettingsService
 *   - Tests GetSettingsAsync: happy path, missing setting, parse failure
 *   - Tests UpdateSettingsAsync: happy path, null dto, update failure
 *   - Uses NSubstitute for ISettingsRepository
 *
 * PRESERVED:
 *   - Test naming convention: [MethodName]_[StateUnderTest]_[ExpectedResult]
 *   - Settings ID constants match VBA source verbatim (InflationExceptional not InflationExceptionalCosts)
 *
...
namespace Apha.Costbook.Application.UnitTests.Services.MaintenanceSettingsServiceTest
{
    public class MaintenanceSettingsServiceTests
    {
        ...
        public async Task UpdateSettingsAsync_ValidDto_CallsRepositoryBulkUpdate()
        {
            ...
            {
                InflationAnimals = 2.5m, InflationExceptionalCosts = 1.8m, InflationStaff = 3.0m, InflationTests = 2.0m,
                CurrentFinancialYear = 2024, WorkingHoursInDay = 7.4m, WorkingDaysInYear = 220m,
                ProfitAnimals = 15.0m, ProfitExceptionalCosts = 12.5m, ProfitStaff = 10.0m, ProfitTests = 8.0m
            };
        ...
        public async Task UpdateSettingsAsync_BuildsDictionaryWithCorrectKeys()
        {
            ...
            var dto = new MaintenanceSettingsDto
            {
                InflationAnimals = 5m, InflationExceptionalCosts = 4m, InflationStaff = 3m, InflationTests = 2m,
                CurrentFinancialYear = 2025, WorkingHoursInDay = 7.5m, WorkingDaysInYear = 225m,
                ProfitAnimals = 20m, ProfitExceptionalCosts = 18m, ProfitStaff = 16m, ProfitTests = 14m
            };
            ...
            string profitAnimals, string profitExceptional, string profitStaff, string profitTests)
        {
            return new List<Settings>
            {
                ...
                new Settings { Id = "WorkingHoursInDay",    Setting = workingHoursInDay,    Userupdateable = true },
                new Settings { Id = "WorkingDaysInYear",    Setting = workingDaysInYear,    Userupdateable = true },
