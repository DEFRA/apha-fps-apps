...
namespace Apha.Costbook.Application.Services
{
    ...
    public class CapsStaffService : ICapsStaffService
    {
        ...
        {
            if (dto is null)
                throw new ArgumentException("StaffDto must not be null.");
            if (string.IsNullOrWhiteSpace(dto.Mnumber))
                throw new ArgumentException("MNumber must not be null or empty.");
            ...
            if (exists)
                throw new ArgumentException($"A CAPS staff member with MNumber '{normalizedMNumber}' already exists.");
