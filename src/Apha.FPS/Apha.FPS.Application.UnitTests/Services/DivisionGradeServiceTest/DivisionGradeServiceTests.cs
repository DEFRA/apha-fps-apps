...
namespace Apha.FPS.Application.UnitTests.Services.DivisionGradeServiceTest
{
    public class DivisionGradeServiceTests
    {
        private readonly IDivisionGradeRepository _mockRepository;
        private readonly IMapper _mockMapper;
        private readonly DivisionGradeService _sut;

        public DivisionGradeServiceTests()
        {
            _mockRepository = Substitute.For<IDivisionGradeRepository>();
            _mockMapper = Substitute.For<IMapper>();
            _sut = new DivisionGradeService(_mockRepository, _mockMapper);
        }
        ...
        private static DivisionGrade BuildEntity(string code = "A-VSD") =>
        ...
        public void Constructor_ThrowsArgumentNullException_WhenRepositoryIsNull()
        {
            Assert.Throws<ArgumentNullException>(() =>
                new DivisionGradeService(null!, _mockMapper));
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() =>
                new DivisionGradeService(_mockRepository, null!));
        }
        ...
        public async Task GetAllPagedAsync_ReturnsPaginatedResult()
        {
            ...
            var pagedData = new PagedData<DivisionGrade>
            {
                Data = new List<DivisionGrade> { BuildEntity() },
                PaginationData = new PaginationData { PageNumber = 1, PageSize = 10, TotalRecords = 1 }
            };
            var pagedResult = new PaginatedResult<DivisionGradeDto>
            {
                Data = new List<DivisionGradeDto> { BuildDto() },
            ...
            _mockMapper.Map<PaginatedResult<DivisionGradeDto>>(pagedData).Returns(pagedResult);
        ...
        public async Task GetAllPagedAsync_ReturnsEmptyResult_WhenNoData()
        {
            ...
            var emptyResult = new PaginatedResult<DivisionGradeDto>
            {
                Data = [],
            ...
            _mockMapper.Map<PaginationParameters<string>>(query).Returns(mappedParams);
            _mockRepository.GetAllPagedAsync(mappedParams).Returns(pagedData);
            _mockMapper.Map<PaginatedResult<DivisionGradeDto>>(pagedData).Returns(emptyResult);
        ...
        {
            _mockRepository.GetByIdAsync("NOTEXIST").Returns((DivisionGrade?)null);
        ...
        public async Task GetByIdAsync_ReturnsMappedDto_WhenFound()
        {
            ...
            _mockMapper.Map<DivisionGradeDto>(entity).Returns(dto);
        ...
        public async Task CreateAsync_ReturnsMappedDto_WhenSuccessful()
        {
            ...
            _mockMapper.Map<DivisionGrade>(dto).Returns(entity);
            _mockRepository.CreateAsync(entity).Returns(created);
            _mockMapper.Map<DivisionGradeDto>(created).Returns(dto);
        ...
        public async Task UpdateAsync_ReturnsMappedDto_WhenSuccessful()
        {
            ...
            _mockMapper.Map<DivisionGradeDto>(updated).Returns(dto);
