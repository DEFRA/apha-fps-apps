...
namespace Apha.FPS.DataAccess.UnitTests.Repository.DivisionGradeRepositoryTest
{
    public class DivisionGradeRepositoryTests
    {
        #region Helpers

        private static DivisionGrade BuildDivisionGrade(
            string code = "A-VSD",
            string gradeCode = "A",
            string division = "VSD",
            int fpsYear = 2025,
        ...
        private static DivisionGradeRepository CreateRepository(
            ...
            IEnumerable<Grade>? grades = null)
        {
            ...
            return new DivisionGradeRepository(mockContext.Object);
        }

        private static (
            DivisionGradeRepository Repo,
            Mock<DbSet<DivisionGrade>> DbSet,
            Mock<FpsDbContext> Context)
            CreateRepositoryWithMocks(IEnumerable<DivisionGrade>? divisionGrades = null)
        {
            var mockFpsYearContext = new Mock<IFpsRequestContext>();
            mockFpsYearContext.Setup(x => x.FpsYear).Returns(2025);
            var mockContext = RepositoryTestHelper.CreateMockDbContext<FpsDbContext>(mockFpsYearContext.Object);
            ...
            var repo = new DivisionGradeRepository(mockContext.Object);
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new DivisionGradeRepository(null!));
        }
        ...
        public async Task CreateAsync_SetsYearFromContext()
        {
            ...
            dbSet.Setup(x => x.Add(It.IsAny<DivisionGrade>()));
        ...
        public async Task CreateAsync_AddsEntityAndSavesChanges()
        {
            ...
            dbSet.Verify(x => x.Add(It.Is<DivisionGrade>(d => d.DivisionGradeCode == "NEW-VSD")), Times.Once);
        ...
        public async Task UpdateAsync_DeletesAndReinserts_WhenCodeChanges()
        {
            var existing = BuildDivisionGrade("A-VSD");
            var (repo, dbSet, context) = CreateRepositoryWithMocks([existing]);
            dbSet.Setup(x => x.Remove(It.IsAny<DivisionGrade>()));
            dbSet.Setup(x => x.Add(It.IsAny<DivisionGrade>()));
            ...
            Assert.Equal(2025, result.FpsYear);
            dbSet.Verify(x => x.Remove(It.Is<DivisionGrade>(d => d.DivisionGradeCode == "A-VSD")), Times.Once);
            dbSet.Verify(x => x.Add(It.Is<DivisionGrade>(d => d.DivisionGradeCode == "B-VSD")), Times.Once);
        }
