...
namespace Apha.FPSApps.Infrastructure.UnitTests.Clients.FPS.FpsProjectGroupApiClientTest
{
    public class FpsProjectGroupApiClientTests
    {
        ...
        [Fact]
        public async Task GetProjectsByProjectGroupProjectProfitabilityVLAAsync_WithSuccessResponse_ReturnsMappedProjectList()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var projectGroup = "GRP1";
            ...
            _http.GetAsync<List<ProjectRes>>(Arg.Is<string>(url =>
                    url.Contains("paged-vla/by-project-group") && url.Contains("projectGroup=GRP1")))
                .Returns(apiResponse);
            _mapper.Map<ApiResponseDto<List<ProjectDto>>>(apiResponse).Returns(expectedDto);

            // Act
            var result = await _client.GetProjectsByProjectGroupProjectProfitabilityVLAAsync(query, projectGroup);
        ...
        [Fact]
        public async Task GetProjectsByProjectGroupProjectProfitabilityVLAAsync_WhenApiFails_ReturnsFailureResponse()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var projectGroup = "GRP1";
        ...
        [Fact]
        public async Task GetProjectsByProjectGroupProjectProfitabilityVLAAsync_UsesVlaEndpointUrl()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var projectGroup = "GRP1";
            ...
            // Act
            await _client.GetProjectsByProjectGroupProjectProfitabilityVLAAsync(query, projectGroup);

            // Assert — must use the VLA endpoint, not the standard project-group endpoint
            await _http.Received(1).GetAsync<List<ProjectRes>>(
                Arg.Is<string>(url => url.Contains("paged-vla/by-project-group")));
        }

        [Fact]
        public async Task GetProjectsByProjectGroupProjectProfitabilityVLAAsync_EncodesProjectGroupInUrl()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var projectGroup = "Group A & B";
            ...
            // Assert — raw ampersand must not appear unencoded in the projectGroup path segment
            await _http.Received(1).GetAsync<List<ProjectRes>>(
                Arg.Is<string>(url => !url.Contains("Group A & B")));
        }

        [Fact]
        public async Task GetProjectsByProjectGroupProjectProfitabilityVLAAsync_PassesPaginationQueryParameters()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 3, PageSize = 5, SortBy = "parentproject", Descending = true };
            var projectGroup = "GRP1";
