...
namespace Apha.FPSApps.Infrastructure.UnitTests.Clients.PACT.PactProjectMonthApiClientTest
{
    public class PactProjectMonthApiClientTests
    {
        ...
        [Fact]
        public async Task GetMonthsAsync_WhenApiReturnsSuccess_ReturnsMappedDtoList()
        {
            // Arrange
            var monthResList = new List<MonthRes>
            {
                new() { MonthNumber = 1, MonthName = "January" },
                new() { MonthNumber = 2, MonthName = "February" }
            };
            var apiResponse = new ApiResponse<List<MonthRes>> { Success = true, Data = monthResList };
            var expectedDto = ApiResponseDto<List<MonthDto>>.SuccessResponse(new List<MonthDto>
            {
                new() { Monthnumber = 1, Monthname = "January" },
                new() { Monthnumber = 2, Monthname = "February" }
            });

            _http.GetAsync<List<MonthRes>>(PactApiEndpoints.GetMonths).Returns(apiResponse);
            _mapper.Map<ApiResponseDto<List<MonthDto>>>(apiResponse).Returns(expectedDto);

            // Act
            var result = await _client.GetMonthsAsync();

            // Assert
            Assert.NotNull(result);
            Assert.True(result.Success);
            Assert.Equal(2, result.Data?.Count);
            await _http.Received(1).GetAsync<List<MonthRes>>(PactApiEndpoints.GetMonths);
            _mapper.Received(1).Map<ApiResponseDto<List<MonthDto>>>(apiResponse);
        }

        [Fact]
        public async Task GetMonthsAsync_WhenApiReturnsEmptyList_ReturnsMappedEmptyList()
        {
            // Arrange
            var apiResponse = new ApiResponse<List<MonthRes>> { Success = true, Data = new List<MonthRes>() };
            var expectedDto = ApiResponseDto<List<MonthDto>>.SuccessResponse(new List<MonthDto>());
        ...
        [Fact]
        public async Task GetMonthsAsync_WhenApiReturnsFailure_ReturnsFailureResponse()
        {
            // Arrange
            var errors = new List<ApiError> { new() { Message = "API Error", Code = "API_ERROR" } };
            var apiResponse = new ApiResponse<List<MonthRes>> { Success = false, Errors = errors };
            var mappedResponse = new ApiResponseDto<List<MonthDto>>
            {
                Success = false,
            ...
            _http.GetAsync<List<MonthRes>>(PactApiEndpoints.GetMonths).Returns(apiResponse);
            _mapper.Map<ApiResponseDto<List<MonthDto>>>(apiResponse).Returns(mappedResponse);
