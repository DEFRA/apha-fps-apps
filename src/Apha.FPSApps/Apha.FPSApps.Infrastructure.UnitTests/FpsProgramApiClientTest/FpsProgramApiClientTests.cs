...
namespace Apha.FPSApps.Infrastructure.UnitTests.FpsProgramApiClientTest
{
    public class FpsProgramApiClientTests
    {
        ...
        [Fact]
        public async Task GetAllDirectoratesAsync_WithSuccessResponse_ReturnsDirectorateList()
        {
            // Arrange
            var directorateList = new List<string?> { "IT", "Finance", "HR" };
            var apiResponse = new ApiResponse<List<string?>>
            {
                Success = true,
                Data = directorateList
            };
            var expectedDto = ApiResponseDto<List<string?>>.SuccessResponse(directorateList);

            _http.GetAsync<List<string?>>("api/program/directorates").Returns(apiResponse);
            _mapper.Map<ApiResponseDto<List<string?>>>(apiResponse).Returns(expectedDto);

            // Act
            var result = await _client.GetAllDirectoratesAsync();
            ...
            await _http.Received(1).GetAsync<List<string?>>("api/program/directorates");
        }
        ...
        [Fact]
        public async Task GetAllDirectoratesAsync_WhenExceptionThrown_ReturnsInternalError()
        {
            // Arrange
            _http.GetAsync<List<string?>>("api/program/directorates").ThrowsAsync(new Exception("Network error"));
