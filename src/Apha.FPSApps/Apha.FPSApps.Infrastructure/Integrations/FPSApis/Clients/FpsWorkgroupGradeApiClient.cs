...
{
    /// <summary>
    /// HTTP client implementation for WorkgroupGrade API operations.
    /// </summary>
        ...
        public async Task<ApiResponseDto<List<string>>> GetAllPcGradesAsync()
        {
            ...
            catch (Exception)
            {
                return ApiResponseDto<List<string>>.FailureResponse(
                    new List<ApiErrorDto> { new ApiErrorDto { Message = "Failed to retrieve PC Grades", Code = InternalCodeError } },
                    new ApiMetaDto());
            }
        }
