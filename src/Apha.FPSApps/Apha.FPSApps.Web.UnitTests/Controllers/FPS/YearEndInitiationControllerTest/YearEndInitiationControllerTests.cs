...
namespace Apha.FPSApps.Web.UnitTests.Controllers.FPS.YearEndInitiationControllerTest
{
    public class YearEndInitiationControllerTests
    {
        ...
        private void SetupIndexDefaults(
            ...
            Guid? currentJobExecutionId = null)
        {
            ...
            _yearEndService.GetInitiatedDataSetupJobExecutionIdAsync()
                .Returns(ApiResponseDto<Guid?>.SuccessResponse(currentJobExecutionId));

            _settingService.GetYearEndSettingsAsync(Arg.Any<Guid?>())
            ...
            _monthHourService.GetYearEndMonthHoursAsync(Arg.Any<Guid?>())
        ...
        {
            // Arrange
            SetupIndexDefaults(currentJobExecutionId: null);
        ...
        public async Task Index_WhenActiveRequestExists_CurrentJobExecutionIdIsRestored()
        {
            ...
            SetupIndexDefaults(currentJobExecutionId: jobExecutionId);
        ...
        public async Task LoadConfigValuesGrid_ReturnsPartialViewResult()
        {
            ...
            // Act
            var result = await _controller.LoadConfigValuesGrid(new PaginationFilter<string>(), null);
        ...
        [Fact]
        public async Task LoadConfigValuesGrid_WhenJobExecutionIdIsNull_StillSucceeds()
        {
            // Arrange — pre-Initiate preview path: no active request, grid still loads.
            _settingService.GetYearEndSettingsAsync(null)
                .Returns(ApiResponseDto<List<YearEndSettingDto>>.SuccessResponse([]));
            ...
            await _settingService.Received(1).GetYearEndSettingsAsync(null);
        }

        [Fact]
        public async Task LoadConfigValuesGrid_WhenJobExecutionIdSupplied_PassesItToTheService()
        {
            // Arrange
            var jobExecutionId = Guid.NewGuid();
            _settingService.GetYearEndSettingsAsync(jobExecutionId)
                .Returns(ApiResponseDto<List<YearEndSettingDto>>.SuccessResponse([]));

            // Act
            var result = await _controller.LoadConfigValuesGrid(new PaginationFilter<string>(), jobExecutionId);

            // Assert
            Assert.IsType<PartialViewResult>(result);
            await _settingService.Received(1).GetYearEndSettingsAsync(jobExecutionId);
        }
        ...
        public async Task LoadMonthHoursGrid_ReturnsPartialViewResult()
        {
            ...
            // Act
            var result = await _controller.LoadMonthHoursGrid(new PaginationFilter<string>(), null);
        ...
        [Fact]
        public async Task LoadMonthHoursGrid_WhenJobExecutionIdIsNull_StillSucceeds()
        {
            // Arrange — pre-Initiate preview path: no active request, grid still loads.
            _monthHourService.GetYearEndMonthHoursAsync(null)
                .Returns(ApiResponseDto<List<YearEndMonthHourDto>>.SuccessResponse([]));
            ...
            // Assert
            Assert.IsType<PartialViewResult>(result);
            await _monthHourService.Received(1).GetYearEndMonthHoursAsync(null);
        }

        [Fact]
        public async Task LoadMonthHoursGrid_WhenJobExecutionIdSupplied_PassesItToTheService()
        {
            // Arrange
            var jobExecutionId = Guid.NewGuid();
            _monthHourService.GetYearEndMonthHoursAsync(jobExecutionId)
                .Returns(ApiResponseDto<List<YearEndMonthHourDto>>.SuccessResponse([]));

            // Act
            var result = await _controller.LoadMonthHoursGrid(new PaginationFilter<string>(), jobExecutionId);

            // Assert
            Assert.IsType<PartialViewResult>(result);
            await _monthHourService.Received(1).GetYearEndMonthHoursAsync(jobExecutionId);
        }
        ...
        public async Task EditConfigValue_WhenSettingExists_ReturnsPartialViewWithModel()
        {
            ...
            // Act
            var result = await _controller.EditConfigValue("HoursInDay", null);
        ...
        public async Task EditConfigValue_WhenSettingNotFound_ReturnsNotFound()
        {
            ...
            // Act
            var result = await _controller.EditConfigValue("NonExistentKey", null);
        ...
        public async Task EditConfigValue_IsYesNoTrue_WhenIdContainsApproval()
        {
            ...
            // Act
            var result = await _controller.EditConfigValue("CapApprovalReset", null);
        ...
        [Fact]
        public async Task EditConfigValue_WhenJobExecutionIdSupplied_PassesItToTheService()
        {
            // Arrange
            var jobExecutionId = Guid.NewGuid();
            var settings = new List<YearEndSettingDto>
            ...
            // Act
            await _controller.EditConfigValue("HoursInDay", jobExecutionId);
        ...
        public async Task EditMonthHour_WhenRecordExists_ReturnsPartialViewWithModel()
        {
            ...
            // Act
            var result = await _controller.EditMonthHour(2024, 3, 2024, 3, null);
        ...
        public async Task EditMonthHour_WhenRecordNotFound_ReturnsNotFound()
        {
            ...
            // Act
            var result = await _controller.EditMonthHour(2099, 1, 2099, 1, null);
        ...
        public async Task EditMonthHour_WhenDataIsNull_ReturnsNotFound()
        {
            ...
            // Act
            var result = await _controller.EditMonthHour(2024, 1, 2024, 1, null);
        ...
        [Fact]
        public async Task EditMonthHour_WhenJobExecutionIdSupplied_PassesItToTheService()
        {
            // Arrange
            var jobExecutionId = Guid.NewGuid();
            var records = new List<YearEndMonthHourDto>
            ...
            // Act
            await _controller.EditMonthHour(2024, 1, 2024, 1, jobExecutionId);
        ...
        public async Task SaveSetting_WhenServiceSucceeds_ReturnsJsonWithSuccessTrue()
        {
            ...
            _settingService.SaveSettingAsync(dto, jobExecutionId)
                .Returns(ApiResponseDto<SettingDto>.SuccessResponse(dto));

            // Act
            var result = await _controller.SaveSetting(dto, jobExecutionId);

            // Assert
            var jsonResult = Assert.IsType<JsonResult>(result);
            var value = GetJsonElement(jsonResult);
            Assert.True(value.GetProperty("success").GetBoolean());
            await _settingService.Received(1).SaveSettingAsync(dto, jobExecutionId);
        }
        ...
        public async Task SaveSetting_WhenServiceThrowsException_PropagatesException()
        {
            ...
            var jobExecutionId = Guid.NewGuid();
            var dto = new SettingDto { Id = "Key" };
            _settingService.SaveSettingAsync(dto, jobExecutionId).ThrowsAsync(new Exception("Save failed"));

            // Act & Assert
            var exception = await Assert.ThrowsAsync<Exception>(() => _controller.SaveSetting(dto, jobExecutionId));
            Assert.Equal("Save failed", exception.Message);
        }

        [Fact]
        public async Task SaveSetting_WhenJobExecutionIdIsNull_ReturnsFailureWithoutCallingService()
        {
            // Arrange
            var dto = new SettingDto { Id = "HoursInDay", Setting = "8", FpsYear = 2024 };

            // Act
            var result = await _controller.SaveSetting(dto, null);

            // Assert
            var jsonResult = Assert.IsType<JsonResult>(result);
            var value = GetJsonElement(jsonResult);
            Assert.False(value.GetProperty("success").GetBoolean());
            await _settingService.DidNotReceive().SaveSettingAsync(Arg.Any<SettingDto>(), Arg.Any<Guid>());
        }

        [Fact]
        public async Task SaveSetting_WhenJobExecutionIdIsEmpty_ReturnsFailureWithoutCallingService()
        {
            // Arrange
            var dto = new SettingDto { Id = "HoursInDay", Setting = "8", FpsYear = 2024 };

            // Act
            var result = await _controller.SaveSetting(dto, Guid.Empty);
        ...
        public async Task SaveMonthHour_WhenServiceSucceeds_ReturnsJsonWithSuccessTrue()
        {
            ...
            _monthHourService.SaveMonthHourAsync(dto, jobExecutionId)
                .Returns(ApiResponseDto<MonthHourDto>.SuccessResponse(dto));

            // Act
            var result = await _controller.SaveMonthHour(dto, jobExecutionId);

            // Assert
            var jsonResult = Assert.IsType<JsonResult>(result);
            var value = GetJsonElement(jsonResult);
            Assert.True(value.GetProperty("success").GetBoolean());
            await _monthHourService.Received(1).SaveMonthHourAsync(dto, jobExecutionId);
        }
        ...
        [Fact]
        public async Task SaveMonthHour_WhenJobExecutionIdIsNull_ReturnsFailureWithoutCallingService()
        {
            // Arrange
            var dto = new MonthHourDto { Year = 2024, Month = 3, Days = 20, FpsYear = 2024 };

            // Act
            var result = await _controller.SaveMonthHour(dto, null);

            // Assert
            var jsonResult = Assert.IsType<JsonResult>(result);
            var value = GetJsonElement(jsonResult);
            Assert.False(value.GetProperty("success").GetBoolean());
            await _monthHourService.DidNotReceive().SaveMonthHourAsync(Arg.Any<MonthHourDto>(), Arg.Any<Guid>());
        }

        [Fact]
        public async Task SaveMonthHour_WhenJobExecutionIdIsEmpty_ReturnsFailureWithoutCallingService()
        {
            // Arrange
            var dto = new MonthHourDto { Year = 2024, Month = 3, Days = 20, FpsYear = 2024 };

            // Act
            var result = await _controller.SaveMonthHour(dto, Guid.Empty);
        ...
        public async Task TriggerApprove_WhenServiceSucceeds_ReturnsJsonWithSuccessTrue()
        {
            ...
            const int plannedYear = 2025;
            var jobExecutionId = Guid.NewGuid();
            _yearEndService.TriggerYearEndDataSetupApprovalJobAsync(plannedYear, jobExecutionId)
            ...
            var value = GetJsonElement(jsonResult);
            Assert.True(value.GetProperty("success").GetBoolean());
            await _yearEndService.Received(1).TriggerYearEndDataSetupApprovalJobAsync(plannedYear, jobExecutionId);
        }
        ...
        public async Task TriggerApprove_WhenJobExecutionIdIsNull_ReturnsFailureWithoutCallingService()
        {
            ...
            var value = GetJsonElement(jsonResult);
            Assert.False(value.GetProperty("success").GetBoolean());
            await _yearEndService.DidNotReceive().TriggerYearEndDataSetupApprovalJobAsync(Arg.Any<int>(), Arg.Any<Guid>());
        }
        ...
        public async Task TriggerReject_WhenServiceSucceeds_ReturnsJsonWithSuccessTrue()
        {
            ...
            const int plannedYear = 2025;
            var jobExecutionId = Guid.NewGuid();
            _yearEndService.EnqueueYearEndDataSetupRejectJobAsync(plannedYear, jobExecutionId)
            ...
            var value = GetJsonElement(jsonResult);
            Assert.True(value.GetProperty("success").GetBoolean());
            await _yearEndService.Received(1).EnqueueYearEndDataSetupRejectJobAsync(plannedYear, jobExecutionId);
        }
        ...
        public async Task TriggerReject_WhenJobExecutionIdIsNull_ReturnsFailureWithoutCallingService()
        {
            ...
            var value = GetJsonElement(jsonResult);
            Assert.False(value.GetProperty("success").GetBoolean());
            await _yearEndService.DidNotReceive().EnqueueYearEndDataSetupRejectJobAsync(Arg.Any<int>(), Arg.Any<Guid>());
        }
