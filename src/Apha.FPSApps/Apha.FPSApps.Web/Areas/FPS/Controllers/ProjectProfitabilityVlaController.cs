...
/*
 * TRANSFORMENGINE MIGRATION — ProjectProfitabilityVlaController.cs
 * Pattern  : stack-upgrade/msaccess-frm-to-dotnet10-mvc-e2e  Phase 11 — ViewModels + MVC Controller (Steps 16-17)
 * Migrated : 2026-06-15
 *
 * CHANGED:
 *   - New frontend MVC controller for the Project Profitability VLA page
 *     (source/ui/fps/frmJobcodeTotalsVLA.html prototype).
 *   - Source form: frmJobcodeTotalsVLA — read-only profitability grid with 4 filter
 *     dropdowns (Status, Program, Manager, Customer) and a summary totals bar.
 *   - Index() builds full DataGridConfig explicitly (never left as new()).
 *   - LoadProjectProfitabilityVlaGrid() is the AJAX DataGrid reload endpoint.
 *   - GetProjectProfitabilityVlaSummary() returns JSON summary totals matching
 *     the HTML prototype ppf-total-* readonly inputs (9 aggregate fields).
 *   - PopulateDropdownsAsync() populates 4 filter dropdowns:
 *     StatusList (static), ProgramList (IProgramService), ManagerList (IProjectService),
 *     CustomerList (IProjectService) — lookup flows kept separate from CRUD resource.
 *   - AllowAdd/Edit/Delete = false (JS showAddButton:false; no edit/delete actions).
 *   - Backend binding: GET /api/v1/project/profitability-vla via IProjectService.
 *     GetProjectProfitabilityVlaAsync() — all 4 filter params optional; no placeholder
 *     defaults (parameter compatibility confirmed in Phase 6 backend gate).
 *
 * PRESERVED:
 *   - [Area("FPS")], [Authorize(Roles = "FPSAdmin,FPSUser")],
 *     [AuthorizeForScopes(ScopeKeySection = "FPSApiSettings:Scope")] consistent with all
 *     other FPS MVC controllers.
 *   - Dependency chain: Controller → IProjectService / IProgramService → IFpsApiClient → HTTP.
 *     No repository, DbContext, or API client injected directly.
 *   - ExtraFilterMethod = "getProjectProfitabilityVlaExtraFilters" consistent with other
 *     FPS profitability pages; JS function must be implemented in the Razor view (Phase 12).
 *
 * Phase 14 — Pre-Build Security Review Gate (2026-06-15):
 *   SECURITY FIX: replaced int.MaxValue PageSize in GetProjectProfitabilityVlaSummary
 *   with a bounded constant (SummaryMaxPageSize = 5000). Passing int.MaxValue to the
 *   backend service risks excessive memory allocation and DB load for aggregate-only
 *   operations. 5000 is a safe ceiling for the VLA-scoped dataset; adjust if profiling
 *   indicates a larger set is required.
 *
 * DEFERRED / REQUIRES HUMAN REVIEW:
 *   - TRANSFORMENGINE TODO: confirm Razor view (Phase 12) implements
 *     getProjectProfitabilityVlaExtraFilters() JS function that merges the 4 filter
 *     dropdown values into the DataGrid AJAX POST body.
 *   - TRANSFORMENGINE TODO: confirm summary totals are to be fetched server-side via
 *     GetProjectProfitabilityVlaSummary AJAX call (preferred) or computed client-side
 *     via JS; if client-side only, remove GetProjectProfitabilityVlaSummary action.
 *   - TRANSFORMENGINE TODO: confirm Manager dropdown value/text binding — currently uses
 *     ManagerDto.Name for both Value and Text; verify this matches the backend filter
 *     semantics for the 'manager' query param in GET /api/v1/project/profitability-vla.
 *   - TRANSFORMENGINE TODO: confirm Customer dropdown value/text binding — currently uses
 *     CustomerDto.Customer for both Value and Text.
 *   - TRANSFORMENGINE TODO [Phase 14]: verify SummaryMaxPageSize=5000 is sufficient for
 *     all expected VLA-scoped project rows; increase if aggregate coverage is incomplete.
 */
