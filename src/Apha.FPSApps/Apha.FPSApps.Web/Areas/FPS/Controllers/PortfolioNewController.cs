...
{
    /// <summary>
    /// MVC controller for the Portfolio Details edit form (frmPortfolioNew).
    /// Delegates all data access to <see cref="IProjectService"/> — backend layers are reused in full.
    /// </summary>
    [Area("FPS")]
    ...
    public class PortfolioNewController : Controller
    {
        ...
        public async Task<IActionResult> Index([FromBody] PortfolioNewViewModel model)
        {
            ...
            var response = await _projectService.UpdateProjectAsync(model.ParentProject, dto);
            if (response.Success)
        ...
        private async Task PopulateDropdownsAsync(PortfolioNewViewModel model)
        {
            ...
            await Task.WhenAll(managerTask, diseaseTask, statusTask, customerTask, contractTask, programTask);
            ...
            model.ProgramList = programs
                ...
                .Select(p => new SelectListItem(p.ProgramNo, p.ProgramNo, p.ProgramNo == model.Program))
                .Prepend(new SelectListItem("", ""))
                .ToList();
        }
    }
