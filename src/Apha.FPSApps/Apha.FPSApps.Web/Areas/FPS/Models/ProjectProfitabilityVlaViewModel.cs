...
/*
 * TRANSFORMENGINE MIGRATION — ProjectProfitabilityVlaViewModel.cs
 * Pattern  : stack-upgrade/msaccess-frm-to-dotnet10-mvc-e2e  Phase 11 — ViewModels + MVC Controller (Steps 16-17)
 * Migrated : 2026-06-15
 *
 * CHANGED:
 *   - New ViewModel for the Project Profitability VLA page
 *     (source/ui/fps/frmJobcodeTotalsVLA.html).
 *   - Four page-level filter dropdowns: SelectedStatus (static), SelectedProgram,
 *     SelectedManager, SelectedCustomer — each has an explicit <select> element outside
 *     the grid container in the HTML prototype.
 *   - ProgramList/ManagerList/CustomerList are the SelectListItem collections for the
 *     three dynamic dropdowns; StatusList is a static list built in the controller.
 *   - DataGridConfig<ProjectProfitabilityVlaItem> ProfitabilityVlaGrid holds the grid
 *     configuration; built explicitly in the controller Index() action (never left as new()).
 *   - Summary totals row fields (9 decimal? properties) represent the server-side
 *     aggregation of the summary bar visible in the HTML prototype
 *     (ppf-total-staff-costs, ppf-total-test-cost, etc.).  These are populated by the
 *     GetProjectProfitabilityVlaSummary action and surfaced to the Razor view.
 *
 * PRESERVED:
 *   - Dropdown list property naming convention [FieldName]List consistent with all
 *     other FPS ViewModel classes in this solution.
 *   - No entity or DTO types exposed directly on the ViewModel (controller maps DTOs
 *     to SelectListItem / Item / summary scalar before handing to View).
 *
 * DEFERRED / REQUIRES HUMAN REVIEW:
 *   - TRANSFORMENGINE TODO: confirm SelectedStatus default ("" = All statuses) is correct;
 *     adjust static StatusList option values/text if backend filter contract changes.
 *   - TRANSFORMENGINE TODO: confirm Summary totals are populated server-side (via
 *     GetProjectProfitabilityVlaSummary action) or client-side (JS updateSummary);
 *     if client-side only, remove the Summary* decimal properties from this ViewModel
 *     and compute totals in JavaScript matching projectprofitability_vla.js behaviour.
 *   - TRANSFORMENGINE TODO: Manager dropdown uses ManagerDto.Name as Value and Text;
 *     verify this matches backend VLA query filter semantics (manager name string).
 *   - TRANSFORMENGINE TODO: Customer dropdown uses CustomerDto.Customer as Value and Text;
 *     verify this matches backend VLA query filter semantics (customer name string).
 */
...
namespace Apha.FPSApps.Web.Areas.FPS.Models
{
    ...
    public class ProjectProfitabilityVlaViewModel
    {
        ...
        // TRANSFORMENGINE: filterProjectStatus <select> outside grid — static options
        ...
        // TRANSFORMENGINE: filterProgram <select> outside grid — dynamic; populated from
        ...
        // TRANSFORMENGINE: filterManager <select> outside grid — dynamic; populated from
        ...
        // TRANSFORMENGINE: filterCustomer <select> outside grid — dynamic; populated from
        ...
        // TRANSFORMENGINE: static status options matching HTML prototype
        ...
        // TRANSFORMENGINE: dynamic program dropdown; values from IProgramService
        /// <summary>Dynamic program dropdown options.</summary>
        public List<SelectListItem> ProgramList { get; set; } = new();

        // TRANSFORMENGINE: dynamic manager dropdown; values from IProjectService.GetManagersAsync()
        /// <summary>Dynamic manager dropdown options.</summary>
        public List<SelectListItem> ManagerList { get; set; } = new();

        // TRANSFORMENGINE: dynamic customer dropdown; values from IProjectService.GetAllCustomersAsync()
        /// <summary>Dynamic customer dropdown options.</summary>
        public List<SelectListItem> CustomerList { get; set; } = new();

        // ── DataGrid configuration ────────────────────────────────────────────

        // TRANSFORMENGINE: DataGridConfig must be built explicitly in controller Index() action.
        ...
        // TRANSFORMENGINE: 9 summary totals matching the HTML prototype ppf-total-* readonly
