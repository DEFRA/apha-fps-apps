...
public class ProjectSummaryController : Controller
{
    ...
    public async Task<IActionResult> Index(string projectId)
    {
        ...
        foreach (var yearDto in yearDtos)
        {
            ...
            var testResponse       = await _yearlyDetailsService.GetTestRequirementsAsync(decodedProjectId, year);
            var animalResponse     = await _yearlyDetailsService.GetAnimalRequirementsAsync(decodedProjectId, year);
            var additionalResponse = await _yearlyDetailsService.GetAdditionalCostsAsync(decodedProjectId, year);
            ...
            rows.Add(new ProjectSummaryRow
            {
                ...
                TestCost            = testResponse.Data?.Sum(t => t.TestCost ?? 0) ?? 0,
                AnimalCost          = animalResponse.Data?.Sum(a => a.AnimalCost ?? 0) ?? 0,
                AdditionalCost      = additionalResponse.Data?.Sum(ac => ac.CostEntered) ?? 0,
