...
(function () {
    ...
    // Containers that centre their dialog with flexbox and therefore have to be
    // neutralised while dragging.
    //
    // ".modal" alone was not enough: screens such as PIMS Yearly Financial Data
    // inject a <dialog class="govuk-edit-modal"> (position:fixed; inset:0;
    // display:flex) into the shared #modalPopup wrapper from _Layout.cshtml.
    // That overlay does not carry the "modal" class, so closest(".modal")
    // matched nothing, "is-dragging-host" was never applied, and the overlay
    // kept re-centring the dialog on every layout pass - which reads as "drag
    // does nothing". Screens that already worked still match ".modal" first, so
    // their behaviour is unchanged.
    //
    // #modalPopup is deliberately NOT listed: its display is toggled inline by
    // the layout's MutationObserver, so forcing display on it could leave an
    // invisible overlay covering the page. It does not need neutralising anyway
    // because a position:fixed dialog is out of flow and ignores its centring.
    var DRAG_HOST_SELECTOR = ".modal, .govuk-edit-modal";

    // Frozen so the flex parent cannot resize the dialog mid-drag. min-width /
    // min-height are included because page specific rules like
    // .yfd-modal-min-width-height { min-width: 900px } otherwise keep enforcing
    // a size larger than the width we pin, which throws off the viewport clamp.
    var FROZEN_SIZE_PROPS = ["width", "max-width", "min-width", "min-height"];

    var POSITION_PROPS = ["position", "left", "top", "margin"];
    ...
    // A mousedown+mouseup pair always emits a synthetic "click" afterwards.
    // Without this flag that click reached the backdrop handler below, which
    // called resetModalPosition() and snapped the dialog straight back to the
    // centre the instant the user released the mouse.
    ...
    function beginDrag($dialog) {
        ...
        // The measured size is pinned rather than zeroed so dialogs relying on
        // a min-width/min-height (e.g. .yfd-modal-min-width-height) keep the
        // exact dimensions they had before the drag started.
        var frozenWidth = $dialog.outerWidth();
        var frozenHeight = $dialog.outerHeight();

        setImportant(el, "width", frozenWidth + "px");
        setImportant(el, "max-width", "none");
        setImportant(el, "min-width", frozenWidth + "px");
        setImportant(el, "min-height", frozenHeight + "px");
        setImportant(el, "position", "fixed");
        setImportant(el, "margin", "0");

        // Stop the flex container re-centring the dialog mid-drag.
        $dialog.closest(DRAG_HOST_SELECTOR).addClass("is-dragging-host");
    }
    ...
    // Any fresh press starts a new interaction, so a stale suppression flag
    // (e.g. the mouse was released outside the window and no click followed)
    // can never swallow a later genuine backdrop click.
    ...
    $(document).on("mousemove", function (e) {
        ...
        // Any real movement means the upcoming click is the tail of a drag,
        // not a deliberate backdrop click.
    ...
    $(document).on("click", function (e) {
        ...
        // Swallow the click generated by the drag that has just finished.
    ...
    function resetModalPosition($dialog) {
        ...
        clearProps(el, POSITION_PROPS.concat(FROZEN_SIZE_PROPS));

        $dialog.removeClass("is-dragging");
        $dialog.closest(DRAG_HOST_SELECTOR).removeClass("is-dragging-host");
    }
