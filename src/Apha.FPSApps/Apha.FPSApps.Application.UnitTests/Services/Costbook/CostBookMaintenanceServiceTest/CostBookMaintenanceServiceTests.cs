/*
 * TRANSFORMENGINE MIGRATION — CostBookMaintenanceServiceTests.cs (FPSApps Application UnitTests)
 * Pattern  : stack-upgrade/msaccess-frm-to-dotnet10-mvc-e2e  Phase 13 — Unit Tests - Backend + Frontend xUnit Coverage
 * Migrated : 2026-06-23
 *
 * CHANGED:
 *   - New xUnit test class for CostBookMaintenanceService (thin-delegate pattern)
 *   - Covers GetSettingsAsync, UpdateSettingsAsync, GetAccountCategoriesAsync, UpdateAccountCategoryAsync
 *   - Mocks ICostBookApiClient (aggregate) + ICostBookMaintenanceApiClient (sub-client)
 *   - Verifies delegation: _costBookClient.CostbookMaintenance delegates to ICostBookMaintenanceApiClient
 *
 * PRESERVED:
 *   - Test naming convention: [MethodName]_[StateUnderTest]_[ExpectedResult]
 *   - Constructor pattern mirrors CostBookCustomerServiceTests (existing convention)
 *   - Uses NSubstitute for all mocks (no Moq)
 *
...
namespace Apha.FPSApps.Application.UnitTests.Services.Costbook.CostBookMaintenanceServiceTest
{
    ...
    {
        // TRANSFORMENGINE: Aggregate client + maintenance sub-client — matches thin-delegate wiring in CostBookMaintenanceService
        ...
        public CostBookMaintenanceServiceTests()
        {
            ...
            // TRANSFORMENGINE: Wire sub-client so _costBookClient.CostbookMaintenance returns the mock
