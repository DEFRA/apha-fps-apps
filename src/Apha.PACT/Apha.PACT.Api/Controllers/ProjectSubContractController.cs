...
namespace Apha.PACT.Api.Controllers
{
    ...
    public class ProjectSubContractController : ControllerBase
    {
        ...
        [HttpGet("animals")]
        public async Task<IActionResult> GetAnimalSubContracts([FromQuery] QueryParameters<string> query, [FromQuery] string? project)
        {
            PaginatedResult<ProjectSubContractDto> pagedResult = await _service.GetAnimalSubContractsAsync(query, project);
            return Ok(_mapper.Map<PaginationRes<ProjectSubContractRes>>(pagedResult));
        }

        /// <summary>Retrieves the total Amount for animal-category sub-contracts.</summary>
        [HttpGet("animals/total")]
        public async Task<IActionResult> GetAnimalTotal([FromQuery] string? project)
        {
            decimal total = await _service.GetAnimalTotalAmountAsync(project);
            return Ok(total);
        }
