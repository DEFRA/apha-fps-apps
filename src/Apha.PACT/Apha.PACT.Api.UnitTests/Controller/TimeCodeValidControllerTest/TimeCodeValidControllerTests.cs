...
namespace Apha.PACT.Api.UnitTests.Controller.TimeCodeValidControllerTest
{
    public class TimeCodeValidControllerTests
    {
        ...
        [Fact]
        public async Task GetPagedTest_WithAllParameters_ReturnsOk()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var dtos = new List<TimeCodeValidDto>
            {
                new TimeCodeValidDto { TimeCode = "TC1", WorkGroup = "WG1", ParentProject = "PRJ1", JobCode = "JC1", TestCode = "TST1" }
            };
            var paginationData = new PaginationDto { PageNumber = 1, PageSize = 10, TotalRecords = 1, TotalPages = 1 };
            var serviceResult = new PaginatedResult<TimeCodeValidDto>(dtos, paginationData);
            var expectedResponse = new PaginationRes<TimeCodeValidRes>
            {
                Data = new List<TimeCodeValidRes>
                {
                    new TimeCodeValidRes { TimeCode = "TC1", WorkGroup = "WG1", ParentProject = "PRJ1", JobCode = "JC1", TestCode = "TST1" }
                },
                PaginationData = new Pagination { PageNumber = 1, PageSize = 10, TotalRecords = 1, TotalPages = 1 }
            };

            _serviceMock.GetPagedTimeCodesTestCodeAsync(query, "JC1", "TST1", "PRJ1").Returns(serviceResult);
            _mapperMock.Map<PaginationRes<TimeCodeValidRes>>(serviceResult).Returns(expectedResponse);

            // Act
            var result = await _controller.GetPagedTest(query, "JC1", "TST1", "PRJ1");
        ...
        [Fact]
        public async Task GetPagedTest_WithTestCodeOnly_ReturnsOk()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var dtos = new List<TimeCodeValidDto>
            {
                new TimeCodeValidDto { TimeCode = "TC1", WorkGroup = "WG1", TestCode = "TST1" },
                new TimeCodeValidDto { TimeCode = "TC2", WorkGroup = "WG2", TestCode = "TST1" }
            };
            ...
            var expectedResponse = new PaginationRes<TimeCodeValidRes>
            {
                Data = new List<TimeCodeValidRes>
                {
                    new TimeCodeValidRes { TimeCode = "TC1", WorkGroup = "WG1", TestCode = "TST1" },
                    new TimeCodeValidRes { TimeCode = "TC2", WorkGroup = "WG2", TestCode = "TST1" }
                },
                PaginationData = new Pagination { PageNumber = 1, PageSize = 10, TotalRecords = 2, TotalPages = 1 }
            };

            _serviceMock.GetPagedTimeCodesTestCodeAsync(query, null, "TST1", null).Returns(serviceResult);
            _mapperMock.Map<PaginationRes<TimeCodeValidRes>>(serviceResult).Returns(expectedResponse);

            // Act
            var result = await _controller.GetPagedTest(query, null, "TST1", null);
        ...
        [Fact]
        public async Task GetPagedTest_WithJobCodeAndParentProject_ReturnsOk()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var dtos = new List<TimeCodeValidDto>
            {
                new TimeCodeValidDto { TimeCode = "TC1", WorkGroup = "WG1", ParentProject = "PRJ1", JobCode = "JC1" }
            };
            var paginationData = new PaginationDto { PageNumber = 1, PageSize = 10, TotalRecords = 1, TotalPages = 1 };
            var serviceResult = new PaginatedResult<TimeCodeValidDto>(dtos, paginationData);
            var expectedResponse = new PaginationRes<TimeCodeValidRes>
            {
                Data = new List<TimeCodeValidRes>
                {
                    new TimeCodeValidRes { TimeCode = "TC1", WorkGroup = "WG1", ParentProject = "PRJ1", JobCode = "JC1" }
                },
                PaginationData = new Pagination { PageNumber = 1, PageSize = 10, TotalRecords = 1, TotalPages = 1 }
            };

            _serviceMock.GetPagedTimeCodesTestCodeAsync(query, "JC1", null, "PRJ1").Returns(serviceResult);
            _mapperMock.Map<PaginationRes<TimeCodeValidRes>>(serviceResult).Returns(expectedResponse);

            // Act
            var result = await _controller.GetPagedTest(query, "JC1", null, "PRJ1");
        ...
        [Fact]
        public async Task GetPagedTest_WithNullParameters_ReturnsOk()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var dtos = new List<TimeCodeValidDto>
            {
                new TimeCodeValidDto { TimeCode = "TC1", WorkGroup = "WG1" },
                new TimeCodeValidDto { TimeCode = "TC2", WorkGroup = "WG2" }
            };
            var paginationData = new PaginationDto { PageNumber = 1, PageSize = 10, TotalRecords = 2, TotalPages = 1 };
            var serviceResult = new PaginatedResult<TimeCodeValidDto>(dtos, paginationData);
            var expectedResponse = new PaginationRes<TimeCodeValidRes>
            {
                Data = new List<TimeCodeValidRes>
                {
                    new TimeCodeValidRes { TimeCode = "TC1", WorkGroup = "WG1" },
                    new TimeCodeValidRes { TimeCode = "TC2", WorkGroup = "WG2" }
                },
                PaginationData = new Pagination { PageNumber = 1, PageSize = 10, TotalRecords = 2, TotalPages = 1 }
            };

            _serviceMock.GetPagedTimeCodesTestCodeAsync(query, null, null, null).Returns(serviceResult);
            _mapperMock.Map<PaginationRes<TimeCodeValidRes>>(serviceResult).Returns(expectedResponse);

            // Act
            var result = await _controller.GetPagedTest(query, null, null, null);
        ...
        [Fact]
        public async Task GetPagedTest_WithTestCodeAndParentProject_ReturnsOk()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var dtos = new List<TimeCodeValidDto>
            {
                new TimeCodeValidDto { TimeCode = "TC1", WorkGroup = "WG1", ParentProject = "PRJ1", TestCode = "TST1" }
            };
            var paginationData = new PaginationDto { PageNumber = 1, PageSize = 10, TotalRecords = 1, TotalPages = 1 };
            var serviceResult = new PaginatedResult<TimeCodeValidDto>(dtos, paginationData);
            var expectedResponse = new PaginationRes<TimeCodeValidRes>
            {
                Data = new List<TimeCodeValidRes>
                {
                    new TimeCodeValidRes { TimeCode = "TC1", WorkGroup = "WG1", ParentProject = "PRJ1", TestCode = "TST1" }
                },
                PaginationData = new Pagination { PageNumber = 1, PageSize = 10, TotalRecords = 1, TotalPages = 1 }
            };

            _serviceMock.GetPagedTimeCodesTestCodeAsync(query, null, "TST1", "PRJ1").Returns(serviceResult);
            _mapperMock.Map<PaginationRes<TimeCodeValidRes>>(serviceResult).Returns(expectedResponse);

            // Act
            var result = await _controller.GetPagedTest(query, null, "TST1", "PRJ1");
        ...
        [Fact]
        public async Task GetPagedTest_WithEmptyResult_ReturnsOkWithEmptyList()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            var serviceResult = new PaginatedResult<TimeCodeValidDto>(Enumerable.Empty<TimeCodeValidDto>(), new PaginationDto());
            ...
            _serviceMock.GetPagedTimeCodesTestCodeAsync(query, null, "NONEXISTENT", null).Returns(serviceResult);
            _mapperMock.Map<PaginationRes<TimeCodeValidRes>>(serviceResult).Returns(expectedResponse);

            // Act
            var result = await _controller.GetPagedTest(query, null, "NONEXISTENT", null);
        ...
        [Fact]
        public async Task GetPagedTest_ServiceThrows_PropagatesException()
        {
            // Arrange
            var query = new QueryParameters<string> { Page = 1, PageSize = 10 };
            _serviceMock.GetPagedTimeCodesTestCodeAsync(query, "JC1", "TST1", "PRJ1")
                .ThrowsAsync(new Exception("Service error"));

            // Act & Assert
            await Assert.ThrowsAsync<Exception>(() => _controller.GetPagedTest(query, "JC1", "TST1", "PRJ1"));
        }
