...
public class SummarisedWgTimeService : ISummarisedWgTimeService
{
    private readonly ISummarisedWgTimeRepository _repository;

    public SummarisedWgTimeService(ISummarisedWgTimeRepository repository)
    {
        ...
        string? workGroup)
    {
        ...
        if (!string.IsNullOrEmpty(query.Search))
        {
            dtoList = dtoList.Where(x =>
                ...
                (x.ProfitCentre?.Contains(query.Search, StringComparison.OrdinalIgnoreCase) ?? false)
            ).ToList();
        }
