...
public sealed class YearEndTableRuleMatrixTests
{
    ...
    [Fact]
    public void YearScopedTargetMustBeEmptyEntries_ShouldAllUseClearTargetYearRows()
    {
        var entries = YearEndTableRuleMatrix.Entries
        ...
        Assert.All(entries, e => Assert.Equal(YearEndTableRuleAction.ClearTargetYearRows, e.Action));
    ...
    [Fact]
    public void ClearTargetYearRowsEntries_ShouldNotOverlapCopyToTargetYearEntries()
    {
        ...
        var clearTableNames = YearEndTableRuleMatrix.Entries
            .Where(e => e.Action == YearEndTableRuleAction.ClearTargetYearRows)
            .Select(e => e.TableName);

        Assert.All(clearTableNames, name => Assert.DoesNotContain(name, copyTableNames));
    }
