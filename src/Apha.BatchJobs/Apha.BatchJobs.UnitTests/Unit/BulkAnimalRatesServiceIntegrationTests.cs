...
public sealed class BulkAnimalRatesServiceIntegrationTests : IAsyncLifetime
{
    ...
    private async Task CleanupAsync(Guid jobQueueId, int fpsYear, string[] animalTypes)
    {
        ...
        // Remove test-only prerequisite rows that Staff tests may also have inserted
        // (shared years 2092/2093 would block tblyearmaster deletion via FK otherwise)
    ...
    [SkippableFact]
    public async Task ExecuteAsync_InsertAction_ThrowsUnsupportedActionException()
    {
        Skip.IfNot(CanRunIntegrationTests(), _skipReason ?? "Integration DB unavailable.");
        await AssertUnexpectedActionThrowsAsync("Insert", "SA5-A10", 2093);
    }
