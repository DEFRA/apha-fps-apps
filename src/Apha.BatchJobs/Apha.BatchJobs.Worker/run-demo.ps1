...
switch ($choice) {
    "1" {
        Write-Host "`n🚀 Starting Demo Mode (NoDb)..." -ForegroundColor Green
    ...
    "2" {
        Write-Host "`n🚀 Starting Production Mode (WithDb)..." -ForegroundColor Magenta
        $env:ASPNETCORE_ENVIRONMENT = "Production"
        Write-Host "   Environment: Production" -ForegroundColor Gray
        Write-Host "   Database: PostgreSQL (Connection string required)" -ForegroundColor Gray
    }
    "3" {
        Write-Host "`n🚀 Starting Development Mode (WithDb)..." -ForegroundColor Blue
