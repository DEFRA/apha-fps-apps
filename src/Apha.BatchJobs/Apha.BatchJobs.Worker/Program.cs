...
try
{
    ...
    // Internal runtime execution key generated by worker on startup.
    var jobQueueId = Guid.NewGuid();
    // Deprecated input kept for transition visibility; worker now owns JobQueueId generation.
    ...
    {
        logger.LogWarning(
            "BATCH_JOBQUEUE_ID was provided but is ignored. Worker now generates JobQueueId internally | ProvidedValue={ProvidedJobQueueId}",
            jobQueueIdEnv);
    }
    ...
    logger.LogInformation("Requested job: {JobName} | RunMode: {RunMode} | JobExecutionId={JobExecutionId} | JobQueueId={JobQueueId} | UserId={UserId}", jobName, runMode, jobExecutionId, jobQueueId, userId);
    ...
    else
    {
        ...
        var result = await orchestrator.RunAsync(jobName, runMode, jobExecutionId, jobQueueId, userId, linkedCts.Token);
