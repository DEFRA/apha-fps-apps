...
/// <summary>Immutable execution context for BulkRates worker services. JobExecutionId and JobName come from the execution request JobOrchestrator already resolved for this run (the correlation id it set, and the job's own <c>Name</c>) rather than re-parsing BATCH_JOB_EXECUTION_ID/BATCH_JOB_NAME from the environment a second time. TriggerYear has no existing scoped source — it is job-specific payload data used only for early mismatch detection, so it is still read from BATCH_JOB_PARAMETERS_JSON.</summary>
public sealed record BulkRatesExecutionContext(
    Guid JobExecutionId,
    string JobName,
    int? TriggerYear)
{
    /// <summary>Builds the execution context from the already-resolved correlation id and job name.</summary>
