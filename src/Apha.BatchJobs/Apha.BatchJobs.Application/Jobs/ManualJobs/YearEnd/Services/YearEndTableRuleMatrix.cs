...
public static class YearEndTableRuleMatrix
{
    ...
    [
        ...
        new(Schema, "tblstaffjob", YearEndTableRole.YearScopedBusinessParticipant, YearEndTableRuleAction.CopyToTargetYear, ["staffid", "jobcode", "fpsyear"],
            ...
            Notes: "FK-dependent on tblwgemployee (staffid) and tlkpproject (jobcode), hence CopyOrder 5, the deepest layer. InactiveEmployeeCleanupStep (pipeline step 9) deletes this table's applicable inactive-employee target-year rows first (FK precedes tblwgemployee's own deletion) — hence FinalRowCountRule.AtMostSource, not MatchSource. Redesigned 2026-08-14 around the legacy Annual_WGEmployeeList.sql rule (personstatus='I' AND enddate IS NULL, General Staff exemption spnumber LIKE 'G%' AND firstname='GENERAL').",
