...
CREATE OR REPLACE VIEW fps.qrysurvprojectsum AS
 SELECT DISTINCT qrytcc_union.project,
   ...
   FROM (fps.qrytcc_union
     ...
     RIGHT JOIN fps.timecostcalcs ON ((((tlkpjobcode.parentproject)::text = (timecostcalcs.project)::text) AND ((tlkpjobcode.jobcode)::text = (timecostcalcs.jobcode)::text)))) ON ((((qrytcc_union.project)::text = (timecostcalcs.project)::text) AND ((qrytcc_union.workgroup)::text = (timecostcalcs.workgroup)::text))))
